...
import { 
  ...
  Dog, Briefcase, Box, 
  MessageCircle, MessageSquareOff, 
  ThermometerSun, Wind,
  Car, User
} from 'lucide-react';
...
export const MusicIcon = ({ pref }: { pref: MusicPref }) => {
  switch (pref) {
    case MusicPref.Quiet: return <VolumeX size={16} className="text-gray-500" />;
    case MusicPref.Loud: return <Volume2 size={16} className="text-gray-500" />;
...
export const BaggageIcon = ({ size }: { size: BaggageSize }) => {
    switch(size) {
        case BaggageSize.Hand: return <Briefcase size={16} className="text-gray-500" />;
        case BaggageSize.Suitcase: return <Box size={16} className="text-gray-500" />;
...
export const ACIcon = ({ hasAC }: { hasAC: boolean }) => {
    return hasAC ? <Wind size={16} className="text-blue-400" /> : <ThermometerSun size={16} className="text-orange-400" />;
}
